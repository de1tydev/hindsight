...
type ApiDryRunRefreshMentalModelRequest struct {
	ctx context.Context
	ApiService *MentalModelsAPIService
	bankId string
	mentalModelId string
...
func (r ApiDryRunRefreshMentalModelRequest) Authorization(authorization string) ApiDryRunRefreshMentalModelRequest {
	r.authorization = &authorization
	return r
}

func (r ApiDryRunRefreshMentalModelRequest) Execute() (*MentalModelDryRunRefreshResult, *http.Response, error) {
	return r.ApiService.DryRunRefreshMentalModelExecute(r)
}

/*
DryRunRefreshMentalModel Dry-run mental model refresh (preview, no persistence)
...
It costs the same LLM tokens as a refresh and is validated the same way.
 ...
 @param bankId
 @param mentalModelId
 @return ApiDryRunRefreshMentalModelRequest
*/
func (a *MentalModelsAPIService) DryRunRefreshMentalModel(ctx context.Context, bankId string, mentalModelId string) ApiDryRunRefreshMentalModelRequest {
	return ApiDryRunRefreshMentalModelRequest{
		ApiService: a,
		ctx: ctx,
		bankId: bankId,
...
func (a *MentalModelsAPIService) DryRunRefreshMentalModelExecute(r ApiDryRunRefreshMentalModelRequest) (*MentalModelDryRunRefreshResult, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPost
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *MentalModelDryRunRefreshResult
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "MentalModelsAPIService.DryRunRefreshMentalModel")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/v1/default/banks/{bank_id}/mental-models/{mental_model_id}/dry-run-refresh"
	localVarPath = strings.Replace(localVarPath, "{"+"bank_id"+"}", url.PathEscape(parameterValueToString(r.bankId, "bankId")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"mental_model_id"+"}", url.PathEscape(parameterValueToString(r.mentalModelId, "mentalModelId")), -1)
...
type ApiSearchMentalModelsRequest struct {
	ctx context.Context
	ApiService *MentalModelsAPIService
	bankId string
	searchMentalModelsRequest *SearchMentalModelsRequest
	authorization *string
}

func (r ApiSearchMentalModelsRequest) SearchMentalModelsRequest(searchMentalModelsRequest SearchMentalModelsRequest) ApiSearchMentalModelsRequest {
	r.searchMentalModelsRequest = &searchMentalModelsRequest
	return r
}

func (r ApiSearchMentalModelsRequest) Authorization(authorization string) ApiSearchMentalModelsRequest {
	r.authorization = &authorization
	return r
}

func (r ApiSearchMentalModelsRequest) Execute() (*MentalModelSearchResponse, *http.Response, error) {
	return r.ApiService.SearchMentalModelsExecute(r)
}
...
Semantically search user-curated mental models with a bounded token budget. Designed for low-latency agent context injection.
 ...
 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param bankId
 @return ApiSearchMentalModelsRequest
*/
func (a *MentalModelsAPIService) SearchMentalModels(ctx context.Context, bankId string) ApiSearchMentalModelsRequest {
	return ApiSearchMentalModelsRequest{
		ApiService: a,
		ctx: ctx,
		bankId: bankId,
...
// Execute executes the request
//  @return MentalModelSearchResponse
func (a *MentalModelsAPIService) SearchMentalModelsExecute(r ApiSearchMentalModelsRequest) (*MentalModelSearchResponse, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPost
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *MentalModelSearchResponse
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "MentalModelsAPIService.SearchMentalModels")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/v1/default/banks/{bank_id}/mental-models/search"
	localVarPath = strings.Replace(localVarPath, "{"+"bank_id"+"}", url.PathEscape(parameterValueToString(r.bankId, "bankId")), -1)

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.searchMentalModelsRequest == nil {
		return localVarReturnValue, nil, reportError("searchMentalModelsRequest is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.searchMentalModelsRequest
	req, err := a.client.prepareRequest(r.ctx, localVarPath, localVarHTTPMethod, localVarPostBody, localVarHeaderParams, localVarQueryParams, localVarFormParams, formFiles)
	if err != nil {
		return localVarReturnValue, nil, err
	}
